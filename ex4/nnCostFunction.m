...
% Cost Function
for k = 1:num_labels
    J = J + (1/m) * (-(new_y(:,k))' * log(h(:,k)) - (1-new_y(:,k))' * log(1-h(:,k)));
end
...
Theta2_grad = [(1/m).*delta_2(:,1) (1/m).*delta_2(:,2:end)+(lambda/m).*Theta2(:,2:end)];
Theta1_grad = [(1/m).*delta_1(:,1) (1/m).*delta_1(:,2:end)+(lambda/m).*Theta1(:,2:end)];
